...
<script>
	...
	var locate = L.control.locate().addTo(map);
		...
		var informasi ='<table class="table table-bordered"><tr><th colspan="2"></th></tr><tbody><tr><td>Nama Wisata</td><td>{{ $data->nama_wisata }}</td></tr><tr><td>Jenis Wisata</td><td>{{ $data->jenis }}</td></tr><tr><td>Kecamatan</td><td>{{ $data->kecamatan }}</td></tr><tr><td>Deskripsi</td><td>{{ $data->deskripsi }}</td></tr><tr><td><a href="/detailwisata/{{ $data->id_wisata }}" class="btn btn-xs btn-warning">Detail</a></td><td><a href="https://www.google.com/maps/search/{{ $data->nama_wisata }}" target="_blank" class="btn btn-xs btn-warning">Menuju Map</a></td></tr></tbody></table>';
